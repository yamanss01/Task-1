...
public final class Action
{
    ...
    private void executeAction(Action action, EventScheduler scheduler) {
        switch (action.kind) {
            case ACTIVITY:
            ...
            EventScheduler scheduler)
    {
        Optional<Entity> fairyTarget =
        		Functions.findNearest(world, entity.position, new ArrayList<>(Arrays.asList(EntityKind.STUMP)));
            ...
            EventScheduler scheduler)
    {
        Optional<Entity> target =
        		Functions.findNearest(world, entity.position, new ArrayList<>(Arrays.asList(EntityKind.TREE, EntityKind.SAPLING)));
            ...
            EventScheduler scheduler)
    {
        Optional<Entity> fullTarget =
        		Functions.findNearest(world, entity.position, new ArrayList<>(Arrays.asList(EntityKind.HOUSE)));
	...
	private boolean transformNotFull(Entity entity, WorldModel world, EventScheduler scheduler,
			ImageStore imageStore) {
	...
	private static Entity createDudeFull(String id, Point position, int actionPeriod, int animationPeriod,
	...
	private boolean transformPlant(Entity entity, WorldModel world, EventScheduler scheduler,
			ImageStore imageStore) {
	...
	public boolean transformTree(Entity entity, WorldModel world, EventScheduler scheduler,
			ImageStore imageStore) {
		if (entity.health <= 0) {
			Entity stump = Functions.createStump(entity.id, entity.position, ImageStore.getImageList(imageStore, Functions.STUMP_KEY));
	...
	public boolean transformSapling(Entity entity, WorldModel world, EventScheduler scheduler,
			ImageStore imageStore) {
		if (entity.health <= 0) {
			Entity stump = Functions.createStump(entity.id, entity.position, ImageStore.getImageList(imageStore, Functions.STUMP_KEY));
			...
			Entity tree = Functions.createTree("tree_" + entity.id, entity.position,
					...
					Functions.getNumFromRange(Functions.TREE_HEALTH_MAX, Functions.TREE_HEALTH_MIN), ImageStore.getImageList(imageStore, Functions.TREE_KEY));
	...
	private Entity createSapling(String id, Point position, List<PImage> images) {
